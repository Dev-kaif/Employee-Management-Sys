...
import { Plus, Search, Calendar, User, Filter, CheckSquare } from 'lucide-react';
    ...
    const fetchData = async () => {
      try {
        ...
        toast({
          ...
          description: error.response?.data?.message || 'Failed to load tasks or employees.',
          variant: 'destructive',
        });
      } finally {
        setLoading(false);
  ...
  const handleCreateTask = async () => {
    try {
      const response = await axios.post(`${Backend_Url}/api/tasks/assign`, {
        ...
        scheduledFor: newTask.dueDate ? new Date(newTask.dueDate).toISOString() : undefined,
      });
      setTasks(prev => [{ ...response.data.task, assignedTo: employees.find(emp => emp._id === newTask.assignedTo), assignedBy: { _id: 'current_user_id', username: 'Current User' } }, ...prev]); // Adjust assignedBy with actual user data if available
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-text-secondary mt-1">Manage and track project assignments</p>
        </div>
