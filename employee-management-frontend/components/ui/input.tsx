...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, icon, fullWidth = false, className = "", type, ...props }, ref) => {
    ...
    return (
      <div className={`${fullWidth ? "w-full" : ""} space-y-1.5`}>
        {label && <label className="block text-sm font-medium text-[#374151]">{label}</label>}
        <div className="relative">
          <motion.div
            ...
            `}
          >
            {icon && <div className="pl-3 text-[#9CA3AF] flex items-center">{icon}</div>}
            <input
              ref={ref}
              type={inputType}
              className={`
        ...
        {error && (
          <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-sm text-[#EF4444]">
            {error}
          </motion.p>
        )}
      </div>
